...
{
public:
    static void ReleaseAllLoggers()
    {
        spdlog::drop_all();
    }

    static void SetGlobalErrorHandler(std::function<void(const String& msg)> handler)
    {
...
public:
    void InitConsoleLogger()
    {
        ...
        {
            std::cout << "ToyUtility::Logger: Log init failed: " << ex.what() << std::endl;
        }
    }

    void InitBasicFileLogger(const String& filePath)
    {
        try
        {
            m_Logger = spdlog::basic_logger_mt("basic_file_logger", filePath);
        }
        catch (const spdlog::spdlog_ex& ex)
        {
            std::cout << "ToyUtility::Logger: Log init failed: " << ex.what() << std::endl;
    ...
    // Example: Create a file rotating logger with 5mb size max and 3 rotated files
    //   InitRotatingFileLogger("logs/log.txt", 1048576 * 5, 3)
    void InitRotatingFileLogger(const String& filePath, int size, int fileNumber)
    {
        try
        {
            m_Logger = spdlog::rotating_logger_mt("rotating_logger", filePath, size, fileNumber);
        }
        catch (const spdlog::spdlog_ex& ex)
        {
            std::cout << "ToyUtility::Logger: Log init failed: " << ex.what() << std::endl;
    ...
    // Example: Create a daily logger - a new file is created every day on 2:30am
    //   InitDailyLogger("logs/daily.txt", 2, 30)
    void InitDailyLogger(const String& filePath, int hour, int minute)
    {
        try
        {
            m_Logger = spdlog::daily_logger_mt("daily_logger", filePath, hour, minute);
        }
        catch (const spdlog::spdlog_ex& ex)
        {
            std::cout << "ToyUtility::Logger: Log init failed: " << ex.what() << std::endl;
    ...
    void SetErrorHandler(std::function<void(const String& msg)> handler)
    {
